...
var PeerManager = class {
  ...
  /**
   * Urspruengliches Ziel einer ausgehenden Verbindung.
   *
   * Wichtig fuer DNS/DynDNS:
   *
   *   Ziel beim connect(): yskar-main.dynv6.net:8646
   *   Tatsae chliche Socket-Adresse: 80.145.156.96:8646
   *
   ...
   */
  ausgehendZiele = /* @__PURE__ */ new Map();
  /**
  ...
  besterPeer() {
    ...
    for (const p of this.bereite()) {
      if (!best || p.fremdeArbeit() > best.fremdeArbeit()) {
        best = p;
      }
    }
    return best;
  ...
  fuelleAus() {
    if (!this.laeuft) return;
    if (this.zahlAus() + this.imAufbau.size >= this.maxAus) {
      return;
    }
  ...
  naechstesZiel() {
    const jetzt = Date.now();
    const verbunden = /* @__PURE__ */ new Set();
    ...
    for (const ziel of this.ausgehendZiele.values()) {
      verbunden.add(ziel);
    }
    ...
    kandidaten.sort(
      (a, b) => a.fehlversuche !== b.fehlversuche ? a.fehlversuche - b.fehlversuche : b.gesehen - a.gesehen
    );
    return kandidaten[0];
  }
  ...
  verbinde(host, port) {
    ...
    if (eintrag) {
      eintrag.zuletztVersucht = Date.now();
    }
  ...
  // ------------------------------------------------------------- Gemeinsam
  nimmVerbindung(sock, richtung, zielPort = 0, urspruenglichesZiel) {
      ...
      callbacks: {
        ...
        onClose: (peer, grund) => {
          ...
          if (grund.includes("selbstverbindung") && zielPort > 0) {
            ...
            this.buch.delete(k);
            this.log(
              `${k} ist die eigene Adresse -- aus dem Buch genommen`
            );
          }
  ...
  aufBereit(p, zielPort) {
    ...
    if (port > 0) {
      this.merke(p.host, port);
    }
    p.send("getaddr", new Uint8Array(0));
    this.opt.onReady?.(p);
  }
  aufNachricht(p, f) {
    if (f.command === "getaddr") {
      return this.aufGetAddr(p);
    }
    if (f.command === "addr") {
      return this.aufAddr(p, f.payload);
    }
    this.opt.onMessage?.(p, f);
  }
  aufGetAddr(p) {
    const jetzt = Date.now();
    const liste = [...this.buch.values()].filter(
      (e) => jetzt - e.gesehen < 3 * 60 * 60 * 1e3 && e.fehlversuche === 0
    ).sort((a, b) => b.gesehen - a.gesehen).slice(0, MAX_ADDR).map((e) => ({
      host: e.host,
      port: e.port,
      gesehen: BigInt(Math.floor(e.gesehen / 1e3))
    }));
    if (liste.length > 0) {
      p.send("addr", encodeAddr(liste));
    }
  }
  aufAddr(p, payload) {
    let liste;
  ...
  merke(host, port, gesehen = Date.now()) {
    ...
    if (vorhanden) {
      if (gesehen > vorhanden.gesehen) {
        vorhanden.gesehen = gesehen;
      }
      return;
    }
    if (this.buch.size >= MAX_BUCH) {
      ...
      for (const e of this.buch) {
        if (!aeltester || e[1].gesehen < aeltester[1].gesehen) {
          aeltester = e;
        }
      }
      if (!aeltester || aeltester[1].gesehen >= gesehen) {
        return;
      }
...
var DEFAULT_SEED = "yskar-main.dynv6.net:8646";
...
var NodeCoreApp = class {
  ...
  async startGui() {
    ...
    this.log(`GUI-Server gestartet \xB7 127.0.0.1:${GUI_PORT}`);
  }
  async shutdown() {
    await this.stopNode();
    if (this.guiServer.listening) {
      await new Promise((resolveGui) => this.guiServer.close(() => resolveGui()));
    }
  }
...
<section id="w1"><h1>YSKAR Node Core</h1><p>Dieser Full Node l\xE4uft als eigenst\xE4ndige Windows-Anwendung. Die Blockchain-, Validierungs- und P2P-Implementierung des YSKAR-Repositories bleibt unver\xE4ndert.</p><p class="hint">Der Standard-Seed zeigt auf deinen bereits getesteten Node #1.</p><div class="wizardActions"><span></span><button class="btn primary" id="beginButton">Einrichtung starten</button></div></section>
<section id="w2" class="hidden"><h1>Node konfigurieren</h1><p>Lege Datenordner und Netzwerkports fest. F\xFCr einen normalen Windows-Node k\xF6nnen die Standardwerte beibehalten werden.</p><div class="field"><label>Datenordner</label><input id="dataDir"><div class="hint">Hier wird die lokale chain.db gespeichert.</div></div><div class="field"><label>Node-API-Port (localhost)</label><input id="nodePort" type="number" value="8645"></div><div class="field"><label>P2P-Port</label><input id="p2pPort" type="number" value="8646"></div><div class="field"><label>Seed</label><input id="seed" value="yskar-main.dynv6.net:8646"></div><div id="err" class="hint" style="color:#ef7c86;margin-top:10px"></div><div class="wizardActions"><button class="btn" id="backButton">Zur\xFCck</button><button class="btn primary" id="saveButton">Weiter</button></div></section>
<section id="w3" class="hidden"><h1>Bereit f\xFCr den Start</h1><p>Der Node startet auf YSKAR Mainnet, \xF6ffnet den P2P-Port und synchronisiert die Blockchain.</p><div class="confirm"><div class="small">Datenordner</div><div id="confirmData" class="mono" style="margin-top:6px;word-break:break-all"></div></div><div style="display:flex;gap:7px;margin-top:13px"><span class="pill">Mainnet</span><span class="pill">P2P 8646</span></div><div class="wizardActions"><button class="btn" id="backConfigButton">Zur\xFCck</button><button class="btn primary" id="startButton">Full Node starten</button></div></section>
</div></div>
<div id="app" class="app hidden">
<aside class="sidebar"><div class="brand"><div class="brandmark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 3 19 7v10l-7 4-7-4V7l7-4Z"/><path d="m8 10 4-2 4 2-4 2-4-2Zm0 4 4 2 4-2"/></svg></div><div><div class="brandname">YSKAR</div><div class="version">Node Core 0.2.2</div></div></div><nav class="nav"><button class="active" data-view="overview"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><rect x="4" y="4" width="6" height="6" rx="1"/><rect x="14" y="4" width="6" height="6" rx="1"/><rect x="4" y="14" width="6" height="6" rx="1"/><rect x="14" y="14" width="6" height="6" rx="1"/></svg><span>\xDCbersicht</span></button><button data-view="blocks"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M5 7h14M5 12h14M5 17h14"/><circle cx="3" cy="7" r=".7" fill="currentColor"/><circle cx="3" cy="12" r=".7" fill="currentColor"/><circle cx="3" cy="17" r=".7" fill="currentColor"/></svg><span>Blockchain</span></button><button data-view="peers"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><circle cx="8" cy="8" r="3"/><circle cx="16" cy="16" r="3"/><path d="m10.5 10.5 3 3"/></svg><span>Peers</span></button><button data-view="settings"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8Z"/><path d="m4 13-1-1 1-1 1-2-1-1 2-2 1 1 2-1 1-2h2l1 2 2 1 1-1 2 2-1 1 1 2 2 1v2l-2 1-1 2 1 1-2 2-1-1-2 1-1 2h-2l-1-2-2-1-1 1-2-2 1-1-1-2-2-1v-2Z"/></svg><span>Einstellungen</span></button></nav><div class="sidebottom">YSKAR Mainnet<br>Full Node \xB7 P2P</div></aside>
<main class="main"><header class="topbar"><div><div id="pageTitle" class="pageTitle">\xDCbersicht</div><div id="pageSub" class="pageSub">Netzwerkstatus und Synchronisation</div></div><div id="status" class="status"><i class="dot"></i><span>Nicht gestartet</span></div></header>
<div class="content">
<section id="view-overview" class="view active"><div class="toolbar"><div><h1>Netzwerk\xFCbersicht</h1><p>Lokaler YSKAR Full Node</p></div><div class="actions"><button class="btn" id="refreshButton">Aktualisieren</button><button class="btn" id="stopButton">Node stoppen</button><button class="btn danger" id="shutdownButton">Beenden</button></div></div><div class="card section" style="margin-bottom:10px"><div class="sectionhead"><h2>Synchronisation</h2><span id="syncText" class="small">Warte auf Status\u2026</span></div><div class="progress"><i id="progressBar"></i></div><div class="syncrow"><span id="syncLeft">Warte auf Peer</span><span id="syncPct">\u2014</span></div></div><div class="grid"><div class="card stat"><div class="label">Blockh\xF6he</div><div id="height" class="value">\u2014</div></div><div class="card stat"><div class="label">Netzwerkziel</div><div id="target" class="value">\u2014</div></div><div class="card stat"><div class="label">Peers</div><div id="peers" class="value">\u2014</div></div><div class="card stat"><div class="label">Gespeicherte Bl\xF6cke</div><div id="blocks" class="value">\u2014</div></div><div class="card stat"><div class="label">Chain Work</div><div id="work" class="value">\u2014</div></div><div class="card stat"><div class="label">Difficulty</div><div id="difficulty" class="value">\u2014</div></div><div class="card stat"><div class="label">P2P</div><div id="p2p" class="value">\u2014</div></div><div class="card stat"><div class="label">Uptime</div><div id="uptime" class="value">\u2014</div></div><div class="card section half"><div class="sectionhead"><h2>Verbindungen</h2><span id="peerMeta" class="small"></span></div><div id="peerList" class="muted">Keine verbundenen Peers.</div></div><div class="card section half"><div class="sectionhead"><h2>Lokaler Node</h2><span class="small">Konfiguration</span></div><div class="kv"><div class="k">Node API</div><div id="api" class="mono">\u2014</div><div class="k">Datenordner</div><div id="dataPath" class="mono" style="word-break:break-all">\u2014</div><div class="k">Seed</div><div id="seedView" class="mono">\u2014</div></div></div></div></section>
<section id="view-blocks" class="view"><div class="toolbar"><div><h1>Blockchain</h1><p>Die zuletzt gespeicherten Mainnet-Bl\xF6cke</p></div></div><div class="card section"><div class="tablewrap"><table class="table"><thead><tr><th>H\xF6he</th><th>Hash</th><th>Zeit</th><th>TX</th><th>Difficulty</th></tr></thead><tbody id="blockRows"><tr><td colspan="5" class="muted">Noch keine Daten.</td></tr></tbody></table></div></div></section>
...
var UI_WIZARD_2 = UI.replace('<section id="w1">', '<section id="w1" class="hidden">').replace('<section id="w2" class="hidden">', '<section id="w2">');
